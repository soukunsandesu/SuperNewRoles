...
namespace SuperNewRoles
{
    ...
    class AllRoleSetClass
    {
        ...
        public static float GetPlayerCount(RoleId RoleDate)
        {
            switch (RoleDate)
            {
                ...
                case (RoleId.HauntedWolf):
                    ...
                    return CustomOption.CustomOptions.EliminatorPlayerCount.getFloat();
                    //プレイヤーカウント
            }
            return 1;
        }
        ...
        if (!(CustomOption.CustomOptions.EliminatorOption.getString().Replace("0%", "") == ""))
            {
                int OptionDate = int.Parse(CustomOption.CustomOptions.EliminatorOption.getString().Replace("0%", ""));
                RoleId ThisRoleId = RoleId.Eliminator;
                if (OptionDate == 10)
                {
