...
namespace SuperNewRoles
{
    ...
    class AllRoleSetClass
    {
        ...
        public static float GetPlayerCount(RoleId RoleDate)
        {
            switch (RoleDate)
            {
                ...
                case (RoleId.Fox):
                    ...
                    return CustomOption.CustomOptions.SubObserverPlayerCount.getFloat();
                    //プレイヤーカウント
            }
            return 1;
        }
        ...
        if (!(CustomOption.CustomOptions.SubObserverOption.getString().Replace("0%", "") == ""))
            {
                int OptionDate = int.Parse(CustomOption.CustomOptions.SubObserverOption.getString().Replace("0%", ""));
                RoleId ThisRoleId = RoleId.SubObserver;
                if (OptionDate == 10)
                {
                    Crewonepar.Add(ThisRoleId);
                }
